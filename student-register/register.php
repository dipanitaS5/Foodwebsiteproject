...
$s_id = $s_name = $s_email = $s_password = '';
$s_id_err = $s_name_err = $s_email_err = $s_password_err = '';

 if($_SERVER['REQUEST_METHOD'] == "POST"){

    //check if user name is empty
    // if(empty(trim($_POST["userid"]))){
    //     $userid_err = "Student ID can not be blank";
    // }
    // else if(empty(trim($_POST["useremail"]))){
    //     $useremail_err = "email can not be blank";
    // }
...
if(empty(trim($_POST['s_password']))){
     ...
     if(empty($s_id_err) && empty($s_password_err) && empty($s_name_err) && empty($s_email_err)){
        $sql = "INSERT INTO  student(s_id,s_name,s_email,s_password) VALUES (?,?,?,?)";
        $stmt = mysqli_prepare($conn, $sql);
        if($stmt)
        {
            mysqli_stmt_bind_param($stmt,"ssss",$param_s_id, $param_s_name,$param_s_email,$param_s_password);
